...
bool datecheck(string date) {
    if (date.size() != 8) { cout << "ѕроверьте размер введеной даты" << endl; return 0; }
    for (int i = 0; i < 8; i++) {
        if (date[i] > '9' || date[i] < '0') { cout << "ƒл€ ввода используютс€ лишь цифры"<<endl; return 0; }
    }
    return 1;
}
...
bool data_check(int day, int month, int year) {
    ...
    else if ((day > days_in_month[month - 1] || day < 1) && !check) { cout << "”казан неправильный день" << endl; check_d = false; }
